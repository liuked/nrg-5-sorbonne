 ...
 * <aaa_port>
 * <aaa_ip>
 * <num vnf configuration lines>
 * <vnf proto> <vnf ip> <vnf port>
 * ....
 * <dev_id>
 * <number of ifaces>
 * <iface1> <type>
...
static int __nso_layer_init(char *config_file) {
    ...
    int vnf_num = 0;
    fscanf(fp, "%d", &vnf_num);
    nso_layer.bspc_tbl = bspc_table_create();

    int proto, port;
    while(vnf_num--) {
        fscanf(fp, "%x%s%d", &proto, if_name, &port);
        int sockfd = connect_vnf(if_name, port);
        if (sockfd < 0) {
            bspc_table_free(nso_layer.bspc_tbl);
            goto err_ret;
        }
        bspc_entry_t *bspc_e = alloc_bspc_entry(proto, sockfd);
        bspc_table_add(nso_layer.bspc_tbl, bspc_e);
    }
...
static void* __vnf_rx_thread_main(void *arg) {
    ...
    FD_ZERO(&master_fds);
    list_for_each_entry(pos, &nso_layer.bspc_tbl->ll_head, ll) {
        FD_SET(pos->sockfd, &master_fds);
    ...
    while (1) {
        ...
        int rc = select(max_fd + 1, &working_fds, NULL, NULL, NULL);
        packet_t *pkt = NULL;
        if (rc > 0) {
            //read nso packet
            int curr_fd;
            for (curr_fd = max_fd; curr_fd >=0; curr_fd--) {
                if (FD_ISSET(curr_fd, &working_fds)) {
                    ...
                    if (device_id_equal(dst_devid, nso_layer.dev_id)) {
                        ...
                        switch(ntohs(hdr->proto)){
                            case NSO_PROTO_CP_VTSD:
                                ...
                                break;
                            case NSO_PROTO_CP_VSON:
                                son_process_rx(pkt, NULL, NULL, NULL);
                                break;
                            default:
                                LOG_DEBUG("unknown protocol field!\n");
                                break;
                        }
                    } else {
                        //nso_fwd
...
int nso_layer_run(char *config_file) {
    ...
    //rx thread from msg from VNF, launch it
    pthread_create(&nso_layer.vnf_rx_pid, NULL, __vnf_rx_thread_main, NULL);
